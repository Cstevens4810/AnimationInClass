package com.example.cteve.animationinclass

/**
 * Created by cteve on 3/1/2018.
 */
...
(context: Context) : View(context) {
    ...
    private var width = 300.0f //rectangle width private
    var stopX = startX + width //right-most x-coordinate
    ...
    } //called from main when button clicked //starts the animation process /** * Called from main when button clicked. * This method starts the animation process. */
    ...
    fun wander() {
        ...
        when (style) {
            ANIMATOR -> getObjectAnimator(500, "fraction", 0.0f, 1.0f).start()

            RAW -> {
                //no animation - just jump to spot
                fraction = 1.0f
                step()
            }
            ACCELERATOR -> {
                //Accelerate in using AccelerateInterpolator
            ...
            DECELERATE -> {
//Deaccelerate using AccelerateInterpolator
...
//Accelerate in using AccelerateInterpolator
            ...
            COLOR_ACC -> getObjectAnimator(800, "currColor", 0.0f, 1.0f).start()
//local method
            MOVE_RECOLOR -> {
    ...
    /** * getObjectAnimator is called to build the object that controls the animation * * @param duration milliseconds the animation should take * @param variable variable to change at each step of animation * @param initialValue starting value for the variable * @param finalValue final value for the variable * @return the ObjectAnimator that controls the variable's changes */
    private fun getObjectAnimator(duration: Int, variable: String, initialValue: Float, finalValue: Float): ObjectAnimator {
        ...
        return animation
    } // /** * setFraction sets the fraction of the complete distance at each step in the animation * This method is called by the ObjectAnimator * * @param value the current value to use */
    ...
    /** * setCurrColor sets the fraction of the complete distance at each step in the animation * This method is called by the ObjectAnimator * * @param value the current value to use */
    fun setCurrColor(value: Float) {
        colorFraction = value
        currColor = oldColor!!.blend(nextColor, value)
        invalidate()
    } // /** * setAngle sets the current angle to use for a rotation at each step in the animation * This method is called by the ObjectAnimator * * @param value the current value to use */
    ...
    /** * Renders the View * * @param canvas Where to do the rendering */
